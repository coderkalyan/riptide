...
export function WaveCanvas() {
  ...
  onMount(() => {
    let disposed = false;
    // A real device loss rebuilds the whole GPU generation (see boot()); cap +
    // back off the retries so a permanently broken adapter can't spin forever.
    let lostRetries = 0;
    const MAX_LOST_RETRIES = 3;
    // Teardown for the current GPU generation (cancel its rAF + run its cleanups,
    ...
    // loss — there is no "reconnect", every GPU object is invalid, so we rebuild.
    // All persistent state (active signals, cursor, markers, viewport) lives
    ...
    const boot = () => {
      ...
      let presented = false; // first frame of this generation → reset the retry budget
      ...
      initGPU(canvasEl).then(async ({ device, ctx, format }) => {
        ...
        // Tear this generation down and boot() a fresh one, repacking from the
        // untouched native db. reason "destroyed" = our own teardown — ignore it.
        // Once the retry budget is spent, fall back to the static overlay (reload to
        // retry). The budget resets after a generation renders a frame, so isolated
        // transient losses over a long session each get the full allotment.
        device.lost.then((info) => {
          if (disposed || lost || info.reason === "destroyed") return;
          lost = true; // stop this generation's in-flight frame immediately
          runCleanups();
          const desc = `${info.reason || "unknown"}${info.message ? `: ${info.message}` : ""}`;
          if (lostRetries++ < MAX_LOST_RETRIES) {
            console.warn(`WebGPU device lost (${desc}) — rebuilding (attempt ${lostRetries})`);
            setTimeout(() => { if (!disposed) boot(); }, 250 * lostRetries);
          } else {
            console.error("WebGPU device lost repeatedly — giving up; reload the window to retry");
            setGpuError(`GPU device lost (${desc}).`);
          }
        });
        ...
        const frame = (now: number) => {
          if (lost) return; // device gone — this generation is being torn down + rebuilt
          ...
          if (!hasTrace()) {
            ...
            perf.markFirstFrame();
            if (!presented) { presented = true; lostRetries = 0; }
            raf = requestAnimationFrame(frame);
            return;
          }
