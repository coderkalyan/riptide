...
// Instanced quad per line: 2.5 CSS px wide, left edge at x_px (extends right).
...
fn vs_line(@builtin(vertex_index) vi: u32, @builtin(instance_index) ii: u32) -> VertexData {
    ...
    // Line is LEFT-aligned: x_px is the left edge and the line extends
    // `thickness` CSS px to the right. This is the shared convention for all
    // time-aligned lines (ruler notches, grid, cursor, markers) so the left
    // edge lands exactly on the logical time instant. The hover guide appears
    // centered on the pointer because App.tsx biases its tick by half a
    // thickness (see updateHover / tickAtClientX). Top starts inside the flag
    // pill (8 CSS px = pill_h/2 in App.tsx) so the line appears to enter it.
